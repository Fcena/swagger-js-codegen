    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            ...
            _.forEach(method.responses, (obj, key) => {
                ...
                if (obj.headers) {
                  method.responses[key].headers = Object.keys(obj.headers).map(name => (
                    {
                      name,
                      tsType: ts.convertType(obj.headers[name])
                    }
                  ))
                  method.responses[key].hasHeaders = method.responses[key].headers.length > 0
                }
            });
